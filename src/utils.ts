...
export function decodeBech32(str: string): { prefix: string; hex: string } {
  const { prefix, words } = bech32.decode(str as `${string}1${string}`, BECH32_MAX_LENGTH);
...
export function nprofileDecode(nprofile: string): { pubkey: string; relays: string[] } {
  const { prefix, words } = bech32.decode(nprofile as `${string}1${string}`, BECH32_MAX_LENGTH);
